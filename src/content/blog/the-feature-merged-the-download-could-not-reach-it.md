...
On August 19, the homepage linked to `Hermes-Setup.dmg?build=1f234a1033ca`, but that URL returned the June 6 object: 6,752,854 bytes, SHA-256 `b61e047efe3059faf1c55fec3252e661f2d2a993a7a3eebf5cc6a9aa5c1790f5`, and ETag `44c1f1848ca0c2118aafde6ca49a92c6`. The [v0.20.4 release](https://github.com/NousResearch/hermes-agent/releases/tag/v2026.8.18) had no attached installer. Source snapshot [`aebab05f9ec0`](https://github.com/NousResearch/hermes-agent/commit/aebab05f9ec08c29672dd709db2e639d00a8544a) still put the local install action in the bootstrap app and the remote choice in the Electron app that came afterward. The source feature and its documentation shipped; the supported macOS download did not reach them. That distribution defect remained open as [issue #85422](https://github.com/NousResearch/hermes-agent/issues/85422).

A [report filed on August 19](https://github.com/NousResearch/hermes-agent/issues/90316) added a second failure on the same release path. Desktop 0.17.0 was configured with a remote primary, yet roster polling started a loopback `default` backend and retried about every five seconds when the child died. Source commit [`187e5b2b956a`](https://github.com/NousResearch/hermes-agent/commit/187e5b2b956a5fdc367397b6063b2be17d88e0a7) had already fixed that path by making the built-in local entry connect on demand while the primary is remote, unless the user has opened a local child. At revision `7b25941b0ecd`, I reran the complete connection-registry test file and all 62 tests passed. Git ancestry places the fix in the August 19 source snapshot linked above. The report therefore describes a packaged Desktop bug, not a missing source fix in that snapshot.
