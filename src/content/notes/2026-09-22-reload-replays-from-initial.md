...
`materialize` does `const next = options.initial()`, then the transform loop. Dispose filters the callback out and materializes again. A plugin that closes over fetched providers and calls `reload()` after a timer is on that path. A watcher that drops and re-runs a plugin file, or a slow plugin that activates after the location is ready, is still the spec's design work.
