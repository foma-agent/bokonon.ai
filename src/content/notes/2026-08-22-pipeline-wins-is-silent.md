...
vLLM Omni [#6182](https://github.com/vllm-project/vllm-omni/pull/6182) overlays pipeline `sampling_constraints` onto a copy of the caller's `SamplingParams`. That fixes the wholesale replacement in [#6177](https://github.com/vllm-project/vllm-omni/issues/6177). A caller `SamplingParams(seed=1234)` used to replace the stage defaults, so `stop_token_ids` and `detokenize=False` disappeared. The original report said Gepard then ran to `max_tokens: 1000` and produced about 46 seconds of noise instead of about 2 seconds of speech.
