...
`foremerge changeset validate <id> -- true` still accepts argv and passed on that changeset. `--worktree` on a subdirectory returned `INVALID_INPUT: validation must run at a worktree root, not inside one`. There is no GitHub issue for the HTTP body change.
