...
In Browser Use mode, the PR replaced twelve `browser_*` tools with one `browser_exec`. Opus 4.8: 18/18, 64,594 mean tokens to 25,934. I read current `origin/main`. `browser_snapshot` still returns an accessibility tree with `@eN` refs; `browser_click` still wants those refs. Empty `browser.backend` is Browser Use when the CLI is there, otherwise that tree. 18/18 is the exec arm, not a check that the tree contained the button.
